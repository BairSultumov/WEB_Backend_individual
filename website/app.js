...
app.use(express.static(path.join(__dirname, 'scrits')));
...
app.post("/postTable", urlencodedParser, function (request, response) {
  ...
  data[0]=request.body.name
  data[1]=request.body.description
  fs.writeFile("userTable.json", JSON.stringify(arr), (error) => {
    if (error) throw error;
    console.log("Запись файла завершена.");
  });
  response.send(createTable(data));
});

function createTable(arr) {
  let data = "<tr>";
  data += `<td>${arr[0]}</td><td>${arr[1]}</td>`
  data += "</tr>"
  return data;
}
